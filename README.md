...
pnpm dev --filter=sum-api
pnpm dev --filter=sum-diary
...
# 특정 앱/패키지 작업
pnpm dev --filter=sum-chat     # sum-chat만 개발
pnpm build --filter=hua-i18n-sdk  # hua-i18n-sdk만 빌드
pnpm test --filter=sum-api     # sum-api만 테스트
```
...
### Apps
- **sum-chat**: 실시간 채팅 애플리케이션
- **sum-api**: REST API 서버
- **sum-diary**: 감정 다이어리 애플리케이션
...
- **hua-sum-api-sdk**: SUM API 클라이언트 SDK
...
- `VERCEL_PROJECT_ID_SUM_DIARY`: `sum-diary` 프로젝트 Settings → General의 Project ID
- `VERCEL_PROJECT_ID_SUM_API`: `sum-api` 프로젝트 Settings → General의 Project ID
