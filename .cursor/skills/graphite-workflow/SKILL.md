...
gt create -m "feat(apps/sum-diary): migrate to new Button API"
gt create -m "feat(apps/sum-api): migrate to new Button API"
```

**❌ 잘못된 순서:**
```bash
# 앱 변경을 먼저 스택 생성 (의존성 오류 발생)
gt create -m "feat(apps/sum-diary): use new Button API"  # hua-ui 변경 전
gt create -m "feat(packages/hua-ui): update Button API"  # 나중에
```

**병합 순서:**
1. PR 1 (hua-ui) → 먼저 병합
2. PR 2 (sum-diary) → 다음 병합
3. PR 3 (sum-api) → 마지막 병합
...
# 3. 앱들 업데이트 (상위 레벨)
gt create -m "feat(apps/sum-diary): migrate to new i18n API"
gt create -m "feat(apps/sum-api): migrate to new i18n API"
gt create -m "feat(apps/sum-chat): migrate to new i18n API"
```
