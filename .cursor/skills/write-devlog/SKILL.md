...
```
IF (데브로그를 작성할 때) THEN
  1. 파일명 형식 확인 (DEVLOG_YYYY-MM-DD_제목.md)
  2. 날짜 확인
  3. 완료된 작업 상세 기록 확인
  4. 변경된 파일 목록 확인
  5. README 업데이트 확인
...
```
IF (중요한 작업 완료) THEN
  → "데브로그를 작성하시겠어요? pnpm generate:devlog 명령어를 사용할 수 있습니다."
END IF
```
...
**변경된 파일**:
- `apps/sum-diary/app/components/Example.tsx`
- `apps/sum-diary/app/api/example/route.ts`
