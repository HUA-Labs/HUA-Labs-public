/**
 * @hua-labs/hua-ux/framework - SSR Translations Utils
 * 
 ...
 * 
 * @param config - HuaUxConfig 객체 (i18n 설정 포함)
...
export async function getSSRTranslations(
  ...
  // HuaUxConfig의 supportedLanguages는 string[]이므로 그대로 사용
    ...
    for (const namespace of namespaces) {
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.warn(
            `[hua-ux] Failed to load SSR translation for ${lang}/${namespace}:`,
            error
          );
        }
        initialTranslations[lang][namespace] = {};
