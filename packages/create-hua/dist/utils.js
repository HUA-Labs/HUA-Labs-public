...
async function generatePackageJson(projectPath, projectName) {
    ...
    const packageJson = {
        ...
        dependencies: {
            ...
            '@phosphor-icons/react': versions_1.PHOSPHOR_ICONS_VERSION,
...
`;
    ...
    try {
        ...
        console.warn(chalk_1.default.yellow(`⚠️  Failed to generate Tailwind config: ${error instanceof Error ? error.message : String(error)}`));
    }
}
/**
    ...
    if (projectName) {
        ...
        if (opts.aiContext) {
            ...
            if (await fs.pathExists(aiContextPath)) {
                ...
                content = content.replace(/^# hua Project AI Context/, `# ${projectName} - hua Project AI Context\n\n**Project Name**: ${projectName}`);
        ...
        if (opts.claudeContext) {
            ...
            if (await fs.pathExists(claudeContextPath)) {
                ...
                content = content.replace(/^# hua Project Context/, `# ${projectName} - hua Project Context\n\n**Project Name**: ${projectName}`);
...
function displayNextSteps(projectPath, aiContextOptions) {
    ...
    console.log(chalk_1.default.white(`  ${packageManager} install`));
    console.log(chalk_1.default.white(`  ${devCommand}`));
    if (aiContextOptions?.claudeSkills) {
