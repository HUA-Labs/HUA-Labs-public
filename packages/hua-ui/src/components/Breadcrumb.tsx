...
const Breadcrumb = React.forwardRef<HTMLDivElement, BreadcrumbProps>(
  ({ 
    items, 
    separator = <Icon name="chevron-right" size={16} className="text-gray-400 dark:text-gray-500" />,
    className,
    maxItems,
    showHomeIcon = true,
    homeLabel = "홈",
    ...
    const renderItem = (item: BreadcrumbItem, index: number) => {
      ...
      if (isLast) {
        return (
                  <span
          key={index}
          className="text-gray-900 dark:text-gray-400 font-medium truncate"
        >
          {itemContent}
        </span>
        )
      ...
      if (isClickable) {
        return (
          ...
          className="text-gray-600 dark:text-gray-500 hover:text-gray-900 dark:hover:text-gray-300 transition-colors truncate hover:underline"
        >
          {itemContent}
        </a>
        )
