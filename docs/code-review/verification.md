...
**변경 사항:**
- `apps/sum-api/package.json`: `bcryptjs` → `bcrypt@^6.0.0`
- `apps/sum-api/package.json`: `@types/bcryptjs` → `@types/bcrypt@^6.0.0`
...
- ✅ `deploy-sum-api`, `deploy-production` 작업에 빌드 도구 설치 추가
- ✅ Vercel 배포 단계 제거 (Vercel에서 자동 처리)

**Vercel 설정:**
- ✅ `apps/sum-diary/vercel.json`: `installCommand`에 `--ignore-scripts=false` 추가
- ✅ `apps/sum-api/vercel.json`: `installCommand`에 `--ignore-scripts=false` 추가
...
**변경 사항:**
- ✅ `apps/sum-api/lib/supabase.ts`: 지연 초기화 함수로 변경
- ✅ `apps/sum-api/lib/supabase-client.ts`: 지연 초기화 함수로 변경
...
**설정:**
- ✅ `apps/sum-diary/package.json`: `build` 스크립트에 `prisma generate --schema=./prisma/schema.prisma` 추가
...
**Zustand 업그레이드:**
- ✅ `sum-diary`: `zustand@^5.0.8`
- ✅ `sum-api`: `zustand@^5.0.6`
...
**변경 사항:**
- ✅ `apps/sum-chat/src/shared/loadEmotionWords.ts`: `yaml.parse()` 사용
- ✅ `apps/sum-chat/src/shared/rhythm-loader.ts`: `yaml.parse()` 사용
- ✅ `apps/sum-chat/src/shared/loadTones.ts`: `yaml.parse()` 사용
- ✅ `apps/sum-chat/src/shared/loadTiers.ts`: `yaml.parse()` 사용
- ✅ `apps/sum-chat/src/shared/loadModes.ts`: `yaml.parse()` 사용
- ✅ `apps/sum-chat/src/shared/loadEthics.ts`: `yaml.parse()` 사용
