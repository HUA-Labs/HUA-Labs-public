...
### 증상
```
ERR_PNPM_RECURSIVE_RUN_FIRST_FAIL  sum-diary@0.1.0 build: `...`
Exit status 1
```
...
```bash
pnpm --filter=sum-diary... run build
```
...
```javascript
// apps/sum-diary/scripts/vercel-build.mjs
const packages = [
  '@hua-labs/utils',
  '@hua-labs/i18n-core',
  '@hua-labs/i18n-loaders',
