...
`sum-diary` 앱의 middleware가 정상적으로 동작하는지 확인합니다.
...
**변경된 파일:**
- `apps/sum-diary/app/components/layout/providers.tsx` - `ToastProvider` 추가
- `apps/sum-diary/app/components/layout/ClientComponents.tsx` - `ScrollToTop` 래퍼 생성
- `apps/sum-diary/app/layout.tsx` - 클라이언트 컴포넌트 직접 import 제거
...
- `sum-chat`에 사용하지 않는 `pages/_app.tsx` 파일 존재

**해결:**
- 사용하지 않는 `pages/_app.tsx` 파일을 `_app.tsx.bak`으로 이름 변경
- 또는 완전히 삭제 (백업 후)

**파일:** `apps/sum-chat/pages/_app.tsx` → `_app.tsx.bak`
