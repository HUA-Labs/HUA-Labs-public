...
**영향받는 앱:**
- `apps/sum-chat`: `next-auth@4.24.11`
- `apps/sum-diary`: `next-auth@4.24.13`
...
**핵심 기능 검증 (중요)**
- [ ] **NextAuth 동작 확인**:
  - [ ] `sum-chat`: 로그인/로그아웃 정상 동작
  - [ ] `sum-diary`: 로그인/로그아웃 정상 동작
  - [ ] 세션 관리 정상 동작
  - [ ] OAuth 프로바이더 (Kakao, Google) 정상 동작
- [ ] **API Routes 동작 확인**:
  - [ ] `sum-api`: 주요 API 엔드포인트 정상 응답
  - [ ] `sum-diary`: 일기 CRUD API 정상 동작
  - [ ] `sum-chat`: 채팅 API 정상 동작
- [ ] **Middleware 동작 확인**:
  - [ ] `sum-diary/middleware.ts`: Rate limiting 정상 동작
  - [ ] 인증 체크 정상 동작
  - [ ] IP 기반 제한 정상 동작
...
**주의사항:**
- 프로덕션 앱(`sum-api`, `sum-chat`)은 특히 신중하게 진행
...
2. **베타 서비스**: `sum-api`, `sum-chat`은 베타 서비스 중이므로 안정성을 최우선으로 고려합니다.
