...
VERCEL_PROJECT_ID_SUM_DIARY  # sum-diary 프로젝트 ID
VERCEL_PROJECT_ID_SUM_API    # sum-api 프로젝트 ID
...
```yaml
# apps/sum-diary/** 또는 packages/hua-ui/** 변경 시
sum-diary: true
...
if: needs.detect-changes.outputs.sum-diary == 'true' && github.ref == 'refs/heads/develop'
...
### 의존성 패키지 변경
- `packages/hua-ui/**` 변경 시 `sum-diary`도 함께 배포됩니다
