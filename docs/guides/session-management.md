...
gt create -m "feat(sum-diary): add session utils helper functions"

# 스택 2: API 라우트에 적용
gt create -m "refactor(sum-diary): use session utils in API routes"

# 스택 3: 테스트 작성
gt create -m "test(sum-diary): add session utils tests"
...
```bash
pnpm --filter=sum-diary test
```

테스트 파일: `apps/sum-diary/app/lib/__tests__/session-utils-server.test.ts`

## 참고 자료

- [NextAuth 문서](https://next-auth.js.org/)
- [Session Utils 소스 코드](../../apps/sum-diary/app/lib/session-utils-server.ts)
- [Session Utils 테스트](../../apps/sum-diary/app/lib/__tests__/session-utils-server.test.ts)
