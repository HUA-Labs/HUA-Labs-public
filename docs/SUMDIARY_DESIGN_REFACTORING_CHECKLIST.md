...
- [✅ 완료] **A1. Paper / Minimal Theme 구조 확정**
  ...
  - 파일: `apps/sum-diary/app/providers/SumdiThemeProvider.tsx` (이미 구현됨, body 클래스 적용 개선)
  - 의존성: 없음
  - 예상 시간: 2-3시간
...
- [✅ 완료] **A2. 프리텐다드 global 적용 + 고운바탕 @font-face 설정**
  ...
  - 파일: `apps/sum-diary/app/globals.css` 수정 (CSS 변수 추가 및 폰트 적용 로직 개선)
  - 의존성: 없음 (A1과 병렬 가능)
  - 예상 시간: 1-2시간
...
- [✅ 완료] **B1. Input/Textarea/Button/Card 기본 스타일 통일**
  ...
  - 파일: `apps/sum-diary/app/components/common/` 또는 전역 스타일
...
- [✅ 완료] **C1. DiaryWritePage 리팩토링**
  ...
  - 파일: `apps/sum-diary/app/diary/write/page.tsx`
  - 의존성: B1 (기본 컴포넌트 스타일)
  - 예상 시간: 4-5시간
...
- [✅ 완료] **D1. EmotionAnalysisDisplay 레이아웃 개편**
  ...
  - 파일: `apps/sum-diary/app/diary/analysis/page.tsx`
  - 의존성: B1 (기본 컴포넌트 스타일)
  - 예상 시간: 4-5시간
...
- [✅ 완료] **E1. MetricCard/StatsCards 패키지 컴포넌트로 교체**
  - 🟢 표시된 컴포넌트들을 직접 교체
  - 파일: 
    - `apps/sum-diary/app/components/hua-analysis/MetricCard.tsx`
    - `apps/sum-diary/app/components/dashboard/StatsCards.tsx`
  - 의존성: B1 (기본 컴포넌트 스타일)
  - 예상 시간: 2-3시간
...
- [✅ 완료] **E2. Empty/Loading/Error → 패키지 컴포넌트로 통일**
  ...
  - 파일:
    - `apps/sum-diary/app/components/common/EmptyState.tsx`
    - `apps/sum-diary/app/components/common/LoadingState.tsx`
    - `apps/sum-diary/app/components/common/ErrorMessage.tsx`
  - 의존성: B1 (기본 컴포넌트 스타일)
  - 예상 시간: 2-3시간
...
- [✅ 완료] **F1. Header/BottomNavigation 재구축**
  ...
  - 파일:
    - `apps/sum-diary/app/components/layout/Header.tsx`
    - `apps/sum-diary/app/components/layout/BottomNavigation.tsx`
  - 의존성: B1 (기본 컴포넌트 스타일)
  - 예상 시간: 4-5시간
...
- [✅ 완료] **G1. DiaryList 레이아웃 최소 버전 재정렬**
  ...
  - 파일: `apps/sum-diary/app/components/diary/diary-list.tsx`
  - 의존성: B1 (기본 컴포넌트 스타일)
  - 예상 시간: 3-4시간
...
- [✅ 완료] **H1. Calendar 구조 기반 설계 (UI는 간단 버전)**
  ...
  - 파일: `apps/sum-diary/app/components/diary/diary-calendar.tsx`
  - 의존성: B1 (기본 컴포넌트 스타일)
  - 예상 시간: 5-6시간
...
**파일 위치**:
- `apps/sum-diary/app/providers/ThemeProvider.tsx` (신규)
- `apps/sum-diary/app/layout.tsx` (수정)
...
**파일 위치**:
- `apps/sum-diary/app/globals.css` (수정)
- `apps/sum-diary/app/layout.tsx` (수정)
- `apps/sum-diary/public/fonts/` (폰트 파일 배치)
...
**파일 위치**:
- `apps/sum-diary/app/components/common/` (신규 또는 수정)
- 또는 전역 스타일 파일
...
**파일 위치**:
- `apps/sum-diary/app/diary/write/page.tsx`
- `apps/sum-diary/app/components/diary/DiaryEditor.tsx` (관련)
...
**파일 위치**:
- `apps/sum-diary/app/diary/analysis/page.tsx`
- `apps/sum-diary/app/components/diary/emotion-analysis.tsx`
...
**파일 위치**:
- `apps/sum-diary/app/components/diary/diary-list.tsx`
...
**파일 위치**:
- `apps/sum-diary/app/components/diary/diary-calendar.tsx`
- `apps/sum-diary/app/components/calendar/CalendarGrid.tsx` (신규)
- `apps/sum-diary/app/components/calendar/CalendarDayCell.tsx` (신규)
