...
**중복 제거 효과:**
- `useLoading`: sum-api에서만 구현 → 공통 패키지로
- `useAutoScroll`: sum-chat에서만 구현 → 공통 패키지로
...
# 2. 앱에서 사용
cd apps/sum-api && pnpm add @hua-labs/utils
```
...
# 2. 앱에서 사용
cd apps/sum-chat && pnpm add @hua-labs/hooks
```
...
```bash
# 1. 기존 앱에서 마이그레이션
# sum-api, hua-labs-official에서 기존 ScrollToTop 제거
