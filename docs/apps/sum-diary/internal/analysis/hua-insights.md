...
**"이것은 지시도 명령도 아닌 감응 요청"** (sum-api/lib/lite/prompt-generator.ts:108)
...
```typescript
// apps/sum-diary/app/lib/hua-analyzer.ts
...
### 즉시 (오늘/내일)
- [ ] `apps/sum-diary/app/lib/hua-analyzer.ts` 생성
- [ ] 기본 HUA 분석 로직 구현
- [ ] 실제 일기로 테스트
...
```typescript
// apps/sum-diary/app/lib/hua-language-profiles.ts
