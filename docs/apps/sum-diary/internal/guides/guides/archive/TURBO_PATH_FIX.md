...
```json
{
  "buildCommand": "cd ../.. && turbo run build --filter=sum-diary"
}
```

**현재 설정** (sum-diary, sum-api 동일):
```json
{
...
"buildCommand": "cd ../.. && echo $PATH && which pnpm && which node && turbo run build --filter=sum-diary"
```
