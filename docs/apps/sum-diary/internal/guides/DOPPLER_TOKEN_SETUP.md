...
Error: Command "cd ../.. && doppler run --config prd -- corepack pnpm exec turbo run build --filter=sum-diary" exited with 1
```
...
```json
{
  ...
  "buildCommand": "cd ../.. && doppler run --config prd -- corepack pnpm exec turbo run build --filter=sum-diary",
