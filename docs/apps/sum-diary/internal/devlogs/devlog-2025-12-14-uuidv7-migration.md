...
- **생성**: `apps/sum-diary/docs/SCHEMA_REVIEW_GEMINI.md`
- **내용**: 
...
- **생성**: `apps/sum-diary/docs/QUOTA_CONCURRENCY_FIX.md`
...
#### 8.3 문서화
- **생성**: `apps/sum-diary/docs/CROSS_SCHEMA_FK_STRATEGY.md`
...
#### 9.3 문서화
- **생성**: `apps/sum-diary/docs/CRISIS_ALERT_STORAGE_OPTIMIZATION.md`
...
#### 10.2 문서화
- **생성**: `apps/sum-diary/docs/SUBSCRIPTION_MODEL_GUIDE.md`
...
#### 11.5 문서화
- **생성**: `apps/sum-diary/docs/LOG_CLEANUP_SCHEDULER.md`
...
#### 12.3 문서화
- **생성**: `apps/sum-diary/docs/USER_DELETION_ANONYMIZATION.md`
...
#### 13.3 문서화
- **생성**: `apps/sum-diary/docs/SUBSCRIPTION_WEBHOOK_GUIDE.md`
...
- `apps/sum-diary/docs/SCHEMA_REVIEW_GEMINI.md` - Gemini 스키마 리뷰 문서
- `apps/sum-diary/docs/QUOTA_CONCURRENCY_FIX.md` - UserQuota 동시성 이슈 해결 문서
- `apps/sum-diary/docs/IMPLEMENTATION_SUMMARY.md` - 구현 완료 요약
- `apps/sum-diary/docs/LOG_TTL_STRATEGY.md` - 로그 테이블 TTL 전략 문서
- `apps/sum-diary/docs/CROSS_SCHEMA_FK_STRATEGY.md` - Cross-Schema FK 전략 문서
- `apps/sum-diary/docs/CRISIS_ALERT_STORAGE_OPTIMIZATION.md` - CrisisAlert 스토리지 최적화 전략 문서
- `apps/sum-diary/docs/SUBSCRIPTION_MODEL_GUIDE.md` - Subscription 모델 가이드
- `apps/sum-diary/docs/LOG_CLEANUP_SCHEDULER.md` - 로그 정리 배치 작업 스케줄러 가이드
- `apps/sum-diary/docs/USER_DELETION_ANONYMIZATION.md` - 유저 삭제 시 법적 증거 데이터 익명화 가이드
- `apps/sum-diary/docs/SUBSCRIPTION_WEBHOOK_GUIDE.md` - Subscription Webhook 처리 가이드
- `apps/sum-diary/app/api/cron/cleanup-logs/route.ts` - 로그 정리 Cron API Route
- `apps/sum-diary/app/api/user/delete/route.ts` - 유저 삭제 API
- `apps/sum-diary/app/api/webhooks/stripe/route.ts` - Stripe Webhook 처리 API
- `apps/sum-diary/app/lib/cleanup-logs.ts` - 로그 정리 공통 유틸리티
- `apps/sum-diary/app/lib/user-deletion.ts` - 유저 삭제 및 익명화 유틸리티
- `apps/sum-diary/app/lib/subscription-utils.ts` - Subscription 관련 유틸리티
- `apps/sum-diary/scripts/cleanup-logs.ts` - 로그 정리 배치 작업 스크립트 (리팩토링)
...
- `apps/sum-diary/docs/README.md` - 문서 링크 추가
...
### 참고 자료
- [UUIDv7 마이그레이션 계획](../apps/sum-diary/docs/UUIDV7_MIGRATION_PLAN.md)
- [Gemini 스키마 리뷰](../apps/sum-diary/docs/SCHEMA_REVIEW_GEMINI.md)
- [UserQuota 동시성 이슈 해결](../apps/sum-diary/docs/QUOTA_CONCURRENCY_FIX.md)
- [구현 완료 요약](../apps/sum-diary/docs/IMPLEMENTATION_SUMMARY.md)
...
- **변경**: `turbo run build --filter=sum-diary` → `pnpm --filter=sum-diary... run build`
- **목적**: Turbo PATH 문제를 우회하여 안정적인 빌드 보장
- **참고**: sum-api와 동일한 방식으로 통일

#### 4단계: vercel.json 설정 통일
- **변경**: sum-diary의 `installCommand`를 sum-api와 동일하게 맞춤
- **제거**: 불필요한 `rm -rf` 명령 및 `--ignore-scripts=false` 옵션

**최종 vercel.json (sum-diary)**:
```json
{
  "framework": "nextjs",
  "installCommand": "corepack enable && corepack use pnpm@10.24.0 && cd ../.. && corepack pnpm install --frozen-lockfile",
  "buildCommand": "cd ../.. && pnpm --filter=sum-diary... run build",
  "devCommand": "cd ../.. && pnpm --filter sum-diary run dev"
}
```
...
- `apps/sum-diary/vercel.json`: pnpm filter 사용으로 변경, sum-api와 설정 통일
- `package.json`: Turbo 2.6.3 업데이트
- `apps/sum-diary/docs/TURBO_PATH_FIX.md`: 상세 해결 가이드 문서
...
### 관련 파일
- `apps/sum-diary/app/api/contact/route.ts`: Advisory Lock 구현 (모듈화된 함수 사용)
- `apps/sum-diary/app/lib/advisory-lock.ts`: Lock Key 생성 유틸리티 (신규)
- `apps/sum-diary/scripts/tests/test-contact-rate-limit.ts`: Atomicity 테스트 스크립트
- `apps/sum-diary/scripts/tests/README_RATE_LIMIT_TEST.md`: 테스트 가이드
...
**최종 설정** (`apps/sum-diary/vercel.json`):
```json
{
  "framework": "nextjs",
  "installCommand": "corepack enable && corepack use pnpm@10.24.0 && cd ../.. && corepack pnpm install --frozen-lockfile",
...
3. `vercel.json`의 빌드 커맨드를 `turbo run build --filter=sum-diary --env-mode=loose`로 변경
4. 배포 시도
...
- ✅ Vercel 빌드: pnpm filter로 안정적으로 빌드 중 (sum-api, sum-diary 모두 성공)
...
- `apps/sum-diary/vercel.json`: Vercel 빌드 설정 (pnpm filter 사용)
- `apps/sum-api/vercel.json`: 참고용 설정 (동일한 패턴)
- `apps/sum-diary/docs/TURBO_PATH_FIX.md`: 상세 가이드 문서
