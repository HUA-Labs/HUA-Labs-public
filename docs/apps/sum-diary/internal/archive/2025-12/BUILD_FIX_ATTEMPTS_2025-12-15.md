...
**작업 내용**:
- `apps/sum-diary/prisma.config.ts` 파일 생성
...
- `sum-diary` 앱의 빌드 스크립트 확인 필요

**현재 상태**:
- `apps/sum-diary/package.json`: `"build": "prisma generate --schema=./prisma/schema.prisma && next build --webpack"`
- 이미 `--webpack` 플래그가 설정되어 있음 ✅

**분석**:
- `sum-diary`는 이미 webpack을 사용하도록 설정되어 있음
