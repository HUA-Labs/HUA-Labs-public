...
sleep 2
tailscale up --authkey=\${TAILSCALE_AUTH_KEY} --hostname=sum-diary-prod
...
DATABASE_URL=postgresql://user:pass@100.x.x.x:5432/sum-diary
DIRECT_URL=postgresql://user:pass@100.x.x.x:5432/sum-diary

# Tailscale
TAILSCALE_AUTH_KEY=tskey-auth-xxxxx  # Tailscale Admin Console에서 생성

# NextAuth
NEXTAUTH_URL=https://sum-diary.up.railway.app
...
# 접근 제어 (pg_hba.conf)
# Vercel IP 대역만 허용
host    sum-diary    dbuser    0.0.0.0/0    scram-sha-256
```
...
```bash
# SSL 연결 필수!
DATABASE_URL=postgresql://user:pass@your-public-ip:5432/sum-diary?sslmode=require
```
