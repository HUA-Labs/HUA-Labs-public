...
**현재 상태**: ✅ `apps/sum-diary/app/middleware.ts` 존재
...
- [ ] `apps/sum-diary/package.json`에 `engines.node` 있는지 확인 필요
...
### 해결책 4: apps/sum-diary/package.json에 engines.node 추가
...
2. `apps/sum-diary/package.json`에 `engines.node` 추가
3. 재배포
