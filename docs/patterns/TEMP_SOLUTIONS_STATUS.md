...
- `apps/sum-api/next.config.ts`: `ignoreBuildErrors` 없음 ✅
- `apps/sum-diary/next.config.ts`: `ignoreBuildErrors` 없음 ✅
...
**파일**: `apps/sum-api/lib/common/rate-limiter.ts`
...
**파일**: `apps/sum-api/types/swagger-ui-react.d.ts`
...
2. **Supabase 쿼리 타입 - rate-limiter.ts** ✅
   - `apps/sum-api/lib/common/rate-limiter.ts`: 명시적 타입 정의 추가 (ApiKey, ApiUsageLog)
   - **상태**: 모든 `as any` 제거 완료

3. **Supabase 쿼리 타입 - notification-service.ts** ✅
   - `apps/sum-api/lib/services/notification-service.ts`: Notification 타입 정의 추가
   - **상태**: 모든 `as any` 제거 완료

4. **Supabase 쿼리 타입 - credit-service.ts** ✅
   - `apps/sum-api/lib/services/credit-service.ts`: User, Transaction 타입 정의 추가
...
- `apps/sum-api/next.config.ts`: React 19 타입 문제 해결됨 ✅
- `apps/sum-diary/next.config.ts`: React 19 타입 문제 해결됨 ✅
