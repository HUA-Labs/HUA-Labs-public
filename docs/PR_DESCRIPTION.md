...
### 테스트 & 빌드
- [x] 타입 체크가 통과합니다
- [x] 빌드가 성공합니다 (`sum-api`, `sum-chat` 성공)
- [x] 일부 앱의 기존 코드 문제 확인 (`sum-diary`, `i18n-test`, `hua-motion` - 마이그레이션과 무관)
...
- OS: Windows
- Node.js 버전: 확인 필요
- 빌드 테스트: `sum-api`, `sum-chat` 성공

### 빌드 테스트 결과
- `sum-api`: Build successful
- `sum-chat`: Build successful
- `sum-diary`: Type error (existing code issue, unrelated to migration)
