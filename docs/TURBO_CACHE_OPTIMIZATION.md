...
결과:
- hua-ui: 새로 빌드 (5분)
- sum-diary: 캐시 재사용 (10초) ← 이전에는 5분 걸렸음
```

#### 시나리오 2: 앱만 변경
```
변경: apps/sum-diary/app/page.tsx
결과:
- hua-ui: 캐시 재사용 (즉시)
- sum-diary: 새로 빌드 (5분)
```
...
```bash
# 특정 패키지 캐시 삭제
pnpm turbo build --filter=sum-diary --force
...
```bash
# 캐시 히트율 확인
pnpm turbo build --filter=sum-diary

# 출력 예시:
# ✓ hua-ui:build (cache hit, duration 0.1s)
# ✓ sum-diary:build (cache miss, duration 5m 23s)
```
