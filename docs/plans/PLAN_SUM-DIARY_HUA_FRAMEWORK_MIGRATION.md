...
숨다이어리(sum-diary)를 HUA UX Framework로 마이그레이션하여 코드 일관성을 확보하고, 프레임워크 레벨의 기능(i18n, motion, state, icons)을 통합 사용하도록 한다.
...
```typescript
// apps/sum-diary/hua-ux.config.ts
