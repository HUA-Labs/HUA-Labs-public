...
이 문서는 HUA Platform 내 `@hua-labs/i18n-core` 패키지의 실제 사용 사례를 분석하고, 대시보드(paysbypays)와 숨 API(sum-api) 서비스에서의 적용 사례를 바탕으로 패키지를 정교화하기 위한 분석 및 개발 계획을 제시합니다.

### 분석 대상

- **대시보드 (paysbypays)**: 결제 대행사 대시보드에서의 i18n-core 사용 사례
- **숨 API (sum-api)**: API 서비스에서의 i18n-core 사용 사례
...
### 2. 숨 API (sum-api) 사용 사례

#### 2.1 설정 구조

**파일 위치**: `apps/sum-api/lib/i18n-config.ts`
...
**파일 위치**: `apps/sum-api/app/api/translations/[language]/[namespace]/route.ts`
...
| 항목 | 대시보드 (paysbypays) | 숨 API (sum-api) |
|------|----------------------|------------------|
