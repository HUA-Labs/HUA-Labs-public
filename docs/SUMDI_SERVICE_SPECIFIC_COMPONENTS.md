...
**숨다 서비스의 UI 리팩토링을 위한 단일 진실의 출처(Single Source of Truth) 문서이며, 패키지(`@hua-labs/ui`)와 앱(`sum-diary`/`sum-chat`)의 컴포넌트 책임 구분을 명확히 한다.**
...
- **리팩토링 상태**: 🟡 **패키지 기반 리팩토링**
- **위치**: `apps/sum-diary/app/diary/write/page.tsx`
...
##### `DiaryEditor` (일기 에디터)
- **리팩토링 상태**: 🟡 **패키지 기반 리팩토링**
- **위치**: `apps/sum-diary/app/components/diary/`
- **현재 상태**: 기능 구현 완료, UI 개선 필요
- **리팩토링 방향**:
...
- **리팩토링 상태**: 🟡 **패키지 기반 리팩토링**
- **위치**: `apps/sum-diary/app/components/diary/emotion-analysis.tsx`
...
- **리팩토링 상태**: 🟡 **패키지 기반 리팩토링**
- **위치**: `apps/sum-diary/app/diary/analysis/page.tsx`
...
- **리팩토링 상태**: 🟡 **패키지 기반 리팩토링**
- **위치**: `apps/sum-diary/app/components/hua-analysis/MetricsModal.tsx`
...
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-diary/app/components/hua-analysis/DominantEmotionSection.tsx`
- **역할**: 주요 감정 시각화
- **필수 기능**:
  - 주요 감정 표시
  - 감정 비율 차트
...
##### `MetricCard` (메트릭 카드)
- **리팩토링 상태**: 🟢 **패키지로 교체 가능**
- **위치**: `apps/sum-diary/app/components/hua-analysis/MetricCard.tsx`
...
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-diary/app/components/hua-analysis/SentimentScoreCard.tsx`
- **역할**: 감정 점수 시각화
- **필수 기능**:
  - 점수 표시
  - 점수 해석
...
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-diary/app/components/hua-analysis/ReasoningSection.tsx`
- **역할**: AI 추론 과정 표시
- **필수 기능**:
  - 추론 과정 텍스트
  - 단계별 표시
...
##### `DiaryList` (일기 목록)
- **리팩토링 상태**: 🟡 **패키지 기반 리팩토링**
- **위치**: `apps/sum-diary/app/components/diary/diary-list.tsx`
...
- **리팩토링 상태**: 🔴 **리팩토링 필요**
- **위치**: `apps/sum-diary/app/components/diary/diary-calendar.tsx`
...
- **상태**: 🔴 **새로 만들기**
- **위치**: `apps/sum-diary/app/components/calendar/CalendarGrid.tsx`
- **역할**: 캘린더 그리드 렌더링
- **필수 기능**:
  - 월별 날짜 그리드
  - 일기 작성일 하이라이트
...
- **상태**: 🔴 **새로 만들기**
- **위치**: `apps/sum-diary/app/components/calendar/CalendarDayCell.tsx`
- **역할**: 개별 날짜 셀
- **필수 기능**:
  - 날짜 표시
  - 일기 존재 여부 표시
...
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-diary/app/components/calendar/CalendarSelectedDayList.tsx`
- **역할**: 선택된 날짜의 일기 목록
- **필수 기능**:
  - 선택된 날짜의 일기 표시
  - 일기 카드 렌더링
...
##### `Dashboard` (대시보드)
- **리팩토링 상태**: 🟡 **패키지 기반 리팩토링**
- **위치**: `apps/sum-diary/app/components/diary/Dashboard.tsx`
...
##### `StatsCards` (통계 카드)
- **리팩토링 상태**: 🟢 **패키지로 교체 가능**
- **위치**: `apps/sum-diary/app/components/dashboard/StatsCards.tsx`
- **현재 상태**: 기능 구현 완료, 패키지 컴포넌트로 직접 교체 가능
- **리팩토링 방향**:
...
##### `RecentDiaries` (최근 일기)
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-diary/app/components/dashboard/RecentDiaries.tsx`
- **역할**: 최근 작성한 일기 목록
- **필수 기능**:
  - 최근 일기 목록 로드
  - 일기 미리보기
...
##### `QuickActions` (빠른 액션)
- **상태**: 🟢 **패키지에서 가져오기**
- **위치**: `apps/sum-diary/app/components/dashboard/QuickActions.tsx`
- **역할**: 빠른 액션 버튼들
- **필수 기능**:
  - 새 일기 작성
  - 일기 목록 보기
...
##### `AnalysisCard` (분석 카드)
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-diary/app/components/diary-list/AnalysisCard.tsx`
- **역할**: 일기 분석 결과 카드
- **특징**:
  - 감정 요약
  - 주요 감정 표시
...
##### `QuestionCard` (질문 카드)
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-diary/app/components/diary-list/QuestionCard.tsx`
- **역할**: 자기성찰 질문 카드
- **필수 기능**:
  - 질문 표시
  - 질문별 일기 링크
...
##### `Header` (헤더)
- **리팩토링 상태**: 🟡 **패키지 기반 리팩토링**
- **위치**: `apps/sum-diary/app/components/layout/Header.tsx`
...
- **리팩토링 상태**: 🔴 **리팩토링 필요**
- **위치**: `apps/sum-diary/app/components/layout/BottomNavigation.tsx`
...
##### `Footer` (푸터)
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-diary/app/components/layout/Footer.tsx`
- **역할**: 공통 푸터
- **필수 기능**:
  - 링크 목록
  - 저작권 정보
...
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-diary/app/components/modal/GuestLimitModal.tsx`
- **역할**: 게스트 사용자 제한 안내
- **필수 기능**:
  - 제한 안내 메시지
  - 로그인 유도
...
##### `SearchModal` (검색 모달)
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-diary/app/components/modal/SearchModal.tsx`
- **역할**: 일기 검색 모달
- **필수 기능**:
  - 검색 입력
  - 검색 결과 표시
...
##### `TermsModal` (약관 모달)
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-diary/app/components/modal/TermsModal.tsx`
- **역할**: 약관 표시 모달
- **필수 기능**:
  - 약관 내용 표시
  - 스크롤 가능한 콘텐츠
...
- **리팩토링 상태**: 🟡 **패키지 기반 리팩토링**
- **위치**: `apps/sum-diary/app/components/modal/ProviderSettingsModal.tsx`
...
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-diary/app/components/profile/ProfileHeader.tsx`
- **역할**: 프로필 페이지 헤더
- **필수 기능**:
  - 사용자 정보 표시
  - 프로필 이미지
...
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-diary/app/components/profile/ProfileInfoSection.tsx`
- **역할**: 프로필 정보 표시
- **필수 기능**:
  - 사용자 정보 표시
  - 편집 폼
...
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-diary/app/components/profile/ProfileStatsSection.tsx`
- **역할**: 사용자 통계 표시
- **필수 기능**:
  - 통계 데이터 표시
  - 차트/그래프
...
##### `EmptyState` (빈 상태)
- **리팩토링 상태**: 🟢 **패키지로 교체 가능**
- **위치**: `apps/sum-diary/app/components/common/EmptyState.tsx`
- **현재 상태**: 기능 구현 완료, 패키지 컴포넌트로 직접 교체 가능
- **리팩토링 방향**:
...
##### `LoadingState` (로딩 상태)
- **리팩토링 상태**: 🟢 **패키지로 교체 가능**
- **위치**: `apps/sum-diary/app/components/common/LoadingState.tsx`
- **현재 상태**: 기능 구현 완료, 패키지 컴포넌트로 직접 교체 가능
- **리팩토링 방향**:
...
##### `ErrorMessage` (에러 메시지)
- **리팩토링 상태**: 🟢 **패키지로 교체 가능**
- **위치**: `apps/sum-diary/app/components/common/ErrorMessage.tsx`
- **현재 상태**: 기능 구현 완료, 패키지 컴포넌트로 직접 교체 가능
- **리팩토링 방향**:
...
- **상태**: 🟢 **패키지에서 가져오기**
- **위치**: `apps/sum-diary/app/components/common/NotificationCard.tsx`
- **역할**: 알림 카드
- **필수 기능**:
  - 알림 내용 표시
  - 읽음/삭제 처리
...
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-diary/app/components/common/JailbreakNotice.tsx`
- **역할**: 윤리 정책 위반 알림
- **필수 기능**:
  - 위반 안내
  - 조치 안내
...
- **상태**: 🔴 **새로 만들기**
- **위치**: `apps/sum-diary/app/components/service/PWAInstallPrompt.tsx`
- **역할**: PWA 설치 유도
- **필수 기능**:
  - 설치 가능 여부 감지
  - 설치 프롬프트 표시
...
- **상태**: 🔴 **새로 만들기**
- **위치**: `apps/sum-diary/app/components/service/ServiceWorkerRegistration.tsx`
- **역할**: 서비스 워커 등록 및 관리
- **필수 기능**:
  - 서비스 워커 등록
  - 업데이트 감지
...
- **상태**: 🔴 **새로 만들기**
- **위치**: `apps/sum-diary/app/components/service/NetworkStatus.tsx`
- **역할**: 네트워크 상태 표시
- **필수 기능**:
  - 온라인/오프라인 감지
  - 상태 표시
...
##### `ChatUI` (메인 채팅 인터페이스)
- **상태**: 🔴 **새로 만들기**
- **위치**: `apps/sum-chat/src/components/ChatUI.tsx`
...
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-chat/src/components/MessageBubble.tsx`
...
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-chat/src/components/chat/ChatInputZone.tsx`
- **역할**: 메시지 입력 및 전송 영역
- **특징**:
  - 실시간 입력 처리
  - 스크롤 다운 버튼
...
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-chat/src/components/chat/ChatMessageList.tsx`
...
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-chat/src/components/SessionSidebar.tsx`
...
##### `SessionHeader` (세션 헤더)
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-chat/src/components/SessionHeader.tsx`
- **역할**: 현재 세션의 헤더 정보 표시
- **특징**:
  - 세션 제목 표시
...
##### `SessionList` (세션 리스트)
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-chat/src/components/SessionList.tsx`
...
##### `SettingsModal` (설정 모달)
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-chat/src/components/SettingsModal.tsx`
...
##### `LoginModal` (로그인 모달)
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-chat/src/components/LoginModal.tsx`
- **역할**: 게스트 사용자 로그인 유도
- **특징**:
  - 게스트 제한 안내
  - 소셜 로그인 연동
...
##### `GuestHome` (게스트 홈)
- **상태**: 🟡 **패키지 활용 + 커스터마이징**
- **위치**: `apps/sum-chat/src/components/GuestHome.tsx`
- **역할**: 게스트 사용자 홈 화면
- **특징**:
  - 서비스 소개
  - 체험 버튼
...
- [숨챗 서비스 기능 요약](../apps/sum-chat/docs/SERVICE-FEATURES-SUMMARY.txt)
