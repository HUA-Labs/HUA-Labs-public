...
- 모든 앱에 적용: `sum-diary`, `sum-api`, `sum-chat`, `hua-ui`, `hua-motion`, `i18n-test`, `hua-email-service`, `nextjs-basic` 예제
...
#### ESLint Config Next
- **sum-chat**: 14.1.0 → **16.0.7**
- **sum-api**: 15.3.4 → **16.0.7**
...
#### 추가 패키지
- **swagger-ui-react**: sum-api에 추가 (API 문서화용)
...
#### 패턴 문서 추가
- `apps/sum-diary/docs/patterns/build-time-module-execution.md`: Next.js 빌드 타임 모듈 실행 문제 해결 패턴 문서화
...
### 린트 테스트
- **sum-diary**: 스킵 (Next.js 16 호환성 이슈로 의도된 설정)
- **sum-api**: 설정 문제 (빌드는 성공)
- **sum-chat**: 스킵 (ESLint 9 호환성 이슈로 의도된 설정)
...
- [Doppler Sync 가이드](./apps/sum-diary/DEPLOYMENT.md#방법-1-doppler-sync-권장-)
