...
| **sum-chat** | `G:\HUA\sum-chat` | 🟡 클로즈 베타 | Next.js + Tailwind | 감응 챗 인터페이스 |
| **sum-diary** | `G:\HUA\sum-diary` | 🟡 개발 예정 | Next.js | 일기앱 |
| **hua-labs** | `G:\HUA\hua-labs` | 🟡 개발 중 | Next.js | 현재 작업 중인 프로젝트 |

### 🔗 의존성 관계

```
sum-chat → hua-api → hua-sdk-lite-v2
sum-diary → hua-api → hua-sdk-lite-v2
...
hua-platform/
├── apps/                    # 애플리케이션들
│   ├── sum-chat/           # 감정 대화 앱 (클로즈 베타)
│   ├── sum-diary/          # 감정 일기 앱 (개발 예정)
