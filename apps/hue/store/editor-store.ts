...
import type { EditorNode, Selection, HoverState, DragState, ViewportState, EventHandlers } from "@/types";
...
interface EditorActions {
  ...
  updateCondition: (nodeId: string, condition: Condition | undefined) => void;
  updateActions: (nodeId: string, actions: EventHandlers | undefined) => void;
...
export const useEditorStore = create<EditorState & EditorActions>()(
  subscribeWithSelector((set, get) => ({
    ...
    updateCondition: (nodeId, condition) =>
      set((state) => {
        // 재귀적으로 노드를 찾아 condition 업데이트
        const updateNode = (node: EditorNode): EditorNode => {
          if (node.id === nodeId) {
            return condition ? { ...node, condition } : (() => { const { condition: _, ...rest } = node; return rest; })();
          }
    ...
    updateActions: (nodeId, actions) =>
      set((state) => {
        // 재귀적으로 노드를 찾아 actions 업데이트
        const updateNode = (node: EditorNode): EditorNode => {
          if (node.id === nodeId) {
            if (actions && Object.keys(actions).length > 0) {
              return { ...node, actions };
            }
            // actions가 없거나 빈 객체면 제거
            const { actions: _, ...rest } = node as EditorNode & { actions?: EventHandlers };
            return rest;
          }
          if (Array.isArray(node.children)) {
            return {
 ...
 * 호버 상태만 구독
 */
export const useHover = () => useEditorStore((state) => state.hover);
